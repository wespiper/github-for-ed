...
export const NotificationCard = ({ 
  ...
  return (
    <div 
      ...
      } ${notification.intervention?.severity ? severityStyles[notification.intervention.severity] : ''} border-l-4`}
    >
      <div className="flex items-start space-x-3">
        <div className="flex-shrink-0">
          <IconComponent 
            size={20} 
            className={
              notification.intervention?.severity === 'critical' ? 'text-red-600' :
              notification.intervention?.severity === 'warning' ? 'text-orange-600' :
              'text-blue-600'
            } 
          />
        </div>
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className={`px-2 py-1 text-xs rounded-full border ${priorityStyles[notification.priority]}`}>
              ...
              <div className="flex items-center space-x-4 text-sm text-gray-500 mb-2">
                ...
                {notification.relatedMetrics?.trend && (
                  <div className="flex items-center space-x-1">
                    {getTrendIcon(notification.relatedMetrics.trend)}
                    <span>{notification.relatedMetrics.trend}</span>
                  </div>
                )}
              </div>
                ...
                <div className="bg-gray-50 rounded p-2 mb-2 text-sm">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      {notification.relatedMetrics.currentValue}
                    </div>
                    {notification.relatedMetrics.previousValue && (
                      <div>
                        <span className="font-medium">Previous: </span>
                        {notification.relatedMetrics.previousValue}
                      </div>
                    )}
                    {notification.relatedMetrics.threshold && (
                      <div>
              ...
              {notification.intervention && (
                <div className="border-t pt-2 mt-2">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-medium text-gray-900">Intervention Details</h4>
                    <button
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <ul className="list-disc list-inside text-gray-600 mt-1 space-y-1">
                          {notification.intervention.suggestedActions.map((action, idx) => (
                      ...
                      {notification.intervention.deadline && (
                        <div>
                          ...
                          <span className="text-gray-600">
                            {new Date(notification.intervention.deadline).toLocaleDateString()}
                          </span>
                        </div>
                      )}

                      {notification.intervention.resolvedAt && (
                        <div className="bg-green-50 border border-green-200 rounded p-2">
                          <div className="flex items-center space-x-2 mb-1">
                            <CheckCircle size={16} className="text-green-600" />
                            <span className="font-medium text-green-800">Resolved</span>
                          </div>
                          <p className="text-green-700 text-sm">{notification.intervention.resolution}</p>
                          {notification.intervention.actionsTaken && notification.intervention.actionsTaken.length > 0 && (
                            <div className="mt-2">
                              <span className="font-medium text-green-700">Actions Taken:</span>
                              <ul className="list-disc list-inside text-green-600 text-sm mt-1">
                                {notification.intervention.actionsTaken.map((action, idx) => (
                                  <li key={idx}>{action}</li>
                                ))}
                              </ul>
                            </div>
                      ...
                      {!notification.intervention.resolvedAt && notification.category === 'educational_intervention' && showActions && (
