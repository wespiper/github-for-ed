...
import { useDocument, useUpdateDocument, useDocumentVersions } from '@/hooks/useDocuments';
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 text-sm text-gray-500">
                  ...
                  {document.chapter && (
                    <>
                      <span>•</span>
                      <span>Chapter: {document.chapter}</span>
                    </>
                  )}
                  {document.lesson && (
                    <>
                      <span>•</span>
                      <span>Lesson: {document.lesson}</span>
                    </>
                  )}
                </div>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          {/* Editor */}
          <div className={`${showVersions ? 'lg:col-span-3' : 'lg:col-span-4'}`}>
            <div className="bg-white rounded-lg shadow-sm">
              <div className="p-6">
                <WritingEditor
                  ...
                  autoSaveInterval={document.settings.autoSaveInterval * 1000}
                  editable={canEdit}
                />
              </div>
            </div>
            ...
            <div className="mt-6 bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2">{document.author.firstName} {document.author.lastName}</span>
                </div>
                <div>
                  <span className="font-medium text-gray-500">Type:</span>
                  <span className="ml-2 capitalize">{document.type}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="ml-2">{new Date(document.metadata.lastEditedAt).toLocaleDateString()}</span>
                </div>
              </div>

              {document.collaborators.length > 0 && (
                <div className="mt-4">
                  <span className="font-medium text-gray-500">Collaborators:</span>
